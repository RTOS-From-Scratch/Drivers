...
void Timer_init(TIMER_MODULE_t timer_module, __Timer_Mode mode, __Timer_Count_Dir dir, uint32_t value)
{
    ...
    IO_REG(__Timer_Addr[timer_module], TIMER_CTL_R) = 0x0;
    ...
    if(mode)
        IO_REG(__Timer_Addr[timer_module], TIMER_TA_M_R) |= 0x2;
    //if timer is one shot mode
    else
        IO_REG(__Timer_Addr[timer_module], TIMER_TA_M_R) = 0x1;
...
{
    IO_REG(__Timer_Addr[timer_module], TIMER_CTL_R) |= (1 << 0);
}

bool Timer_isDone(TIMER_MODULE_t timer_module)
{
    while(!(IO_REG(__Timer_Addr[timer_module], TIMER_MRIS) & (1<< 0)));
...
{
    Timer_init(TIMER0, __periodic_timer, __UP, 0xffffffff);
